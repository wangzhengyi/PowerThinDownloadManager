...
class DownloadDispatcher extends Thread {
     ...
     */
    private final int MAX_REDIRECTS = 7; // can't be more than 7.
                    ...
                    if (!destinationDir.exists()) {
                        ...
                        if (!res) {
                            updateDownloadFailed(request, DownloadManager.ERROR_FILE_ERROR,
                                    "Error in creating destination directory");
                            return;
                        }
                    }
    ...
    private void transferData(DownloadRequest request, InputStream in, OutputStream out) {
        ...
        if (request.isNeedCheckAvailSpace() && ThinDownloadUtil.getDataAvailSpace() < mContentLength) {
            request.cancel();
            request.finish();
            updateDownloadFailed(request, DownloadManager.ERROR_LOW_SPACE, "Download cancelled");
            return;
        }
